...
<main>
    ...
    <div class="mt-2 <?= ($addAppointment ?? true) ? '' : 'd-none' ?>">
        <form method="post" class="d-flex flex-column align-items-center" id="appointmentForm">
            ...
            <select name="idClients" id="client" class="rounded my-2" required>
            ...
            <form class="d-flex flex-column justify-content-center w-auto" method="post">
                ...
                <input type="date" name="day" id="day" class="inputForm rounded my-2" required value="<?= $day ?? date('Y-m-d') ?>" min="<?= date('Y-m-d') ?>" step="1">
                <small <?= ($error['day'] ?? false) ? 'class="text-danger"' : '' ?>><?= $error['day'] ?? '' ?></small>
                
                <!-- Rendez-vous - HOUR -->
                <label for="day" class="mt-2">Heure <span class="registrationRequired">*</span></label>
                <select class="my-2 rounded" class="pe-2" name="hour" id="hour">
                ...
                <!-- Rendez-vous - MINUTES -->
                <label for="day" class="mt-2">Minutes <span class="registrationRequired">*</span></label>
                ...
                <div class="registrationBtns mt-2">
                    <button class="registrationBtn registrerBtn rounded" id="addAppointmentBtn">Ajouter</button>
                </div>
