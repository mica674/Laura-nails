...
class Comment
{
    ...
    public function add(): bool
    {
        ...
        $db = Database::connect();
        // Requête SQL
        $sql = 'INSERT INTO `comments` (`title`, `content`, `quotations`, `id_users`) 
                VALUES (:title, :content, :quotations, :id_users);';
    ...
    public static function get(int|null $id = null): array|object|bool
    {
        ...
        // Requête SQL
        $sql = 'SELECT `id`, `title`, `content`, `quotations`, `id_users`, `created_at`
                FROM `comments`' .
            (($id) ? 'WHERE `id` = :id' : '')
